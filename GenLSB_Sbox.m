% function [Hypothesis]=GenLSBsbox(numberOfRuns)
clc
target_values = 0:255;
...
tic
plaintext = load('inputs.txt');
plaintext = plaintext.inputs;
H = zeros(size(plaintext,1),1);
%Hypothesis = zeros(size(plaintext,1),256);
byteStart = 1;
byteEnd = 16;
keyCandidateStart = 0;
...
for BYTE=byteStart:byteEnd
    ...
    for K = keyCandidateStart:keyCandidateStop
        ...
        H(:,K+1)=bitxor(plaintext(:,BYTE),K);
        a(:,1) = SBOX(H(:,K+1)+1);
%         MultiBitHypothesis (:,K+1) = SBOX(H(:,K+1)+1);
        Hypothesis(:,K+1)=bitget(a,1);
        
    end
%     save(sprintf('MultiBitHypothesis%d.mat',BYTE), 'MultiBitHypothesis');
     save(sprintf('Hypothesis%d.mat',BYTE), 'Hypothesis');
end
toc